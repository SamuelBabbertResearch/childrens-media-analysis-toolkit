...
transition-**boundary** F1 **0.86** (range 0.75–0.91) for `content-t27-solo`, matched
...
**Never call it hard-cut F1.** The headline scores all coded transition
boundaries type-agnostically. The historical 0.855 belongs to the combined
`content-t27-diss` configuration; it is not the default detector's score.
`REFERENCE_BOUNDARY_F1_*` and the exported `boundary_f1` key name that basis.
...
**Words per timed-text minute is reported with timed-text density, or not
at all.** WPM divides by the union of cleaned, word-bearing caption or ASR
intervals, not runtime. These intervals are not validated dialogue timing.
Density reports their share of runtime; retain both measures together.
